...
$mod_path = JPATH_SITE . '/modules/mod_xiroweb_tinlienquan/';
 ...
 * @since  1.6
 */
abstract class ModXirowebTinlienquanHelper
{
	/**
	...
	public static function getList(&$params) {
		...
		$list_newer_pre = self::getListProcess($params,'newer','ASC',$params->get('count_newer', 0));
		if (!is_null($list_newer_pre)) {
			$list_newer_pre = ArrayHelper::sortObjects($list_newer_pre,'created', '-1');
		}
		$list_older = self::getListProcess($params, 'older','DESC',$params->get('count_older', 0));
		$list_newest = self::getListProcess($params,'newest','DESC',$params->get('count_newest', 0));

		$list_newer = self::CompareandRemoveduplicateID($list_newer_pre,$list_newest);
		$result = array();
		if (!empty($list_newest)) {
			$result['newest'] = $list_newest;
		}
		if (!empty($list_newer)) {
			$result['newer'] = $list_newer;
		}
		if (!empty($list_older)) {
			$result['older'] = $list_older;
		}		
		return  $result;
	}


	public static function CompareandRemoveduplicateID($arraya,$arrayb) {
		if (empty($arraya) || empty($arraya)) {return;}
		foreach ($arraya as $key => $arrayaitems) {
			if (self::SearchValueinObject($arrayaitems->id,$arrayb )) {
				 unset($arraya[$key]);
			}
		}
		return $arraya;
	...
	public static function SearchValueinObject($valueID,$array) {
	 ...
	 *
	 * @since  1.6
	 * rangenextorprevious: newer, older, newest
	 */

	public static function getListProcess(&$params,$rangenextorprevious,$directionvalue,$count)
	{
		// Get an instance of the generic articles model
		$articles = JModelLegacy::getInstance('Articlestinlienquan', 'ContentModel', array('ignore_request' => true));
		...
		$articles->setState('rangenextorprevious', $rangenextorprevious);
				...
				switch ($view)
				{
					...
					case 'article' :
							...
							// $catid      = $app->input->getInt('catid');
								...
								$createdofarticle = $item->created;
								$articles->setState('thisarticlecreated', $createdofarticle);
		...
		switch ($ordering)
		{
			...
			default:
				$articles->setState('list.ordering', $ordering);
				//$articles->setState('list.direction', $params->get('article_ordering_direction', 'ASC'));
				$articles->setState('list.direction', $directionvalue);
				break;
		}
