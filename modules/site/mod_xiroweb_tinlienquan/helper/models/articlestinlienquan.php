 ...
 * @since  1.6
 */
class ContentModelArticlestinlienquan extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$thisarticlecreated = $this->getState('thisarticlecreated', '');
		$switchmode = $this->getState('rangenextorprevious', '');
		
		switch ($switchmode) {
			case 'older':
					$query->where(
						$dateField . ' < ' . $db->quote($thisarticlecreated)
					);
				break;
